...
class _Test2State extends State<Test2> {
  ...
  final Map<DateTime, Image> _emotions = {
    ...
    DateTime.utc(2025, 9, 24): Image.asset(angryImage),
  };

  @override
  void initState() {
    super.initState();
    // 화면 처음 들어오면 오늘 날짜가 선택된 상태가 되도록 설정
          ...
          TableCalendar(
            ...
            calendarStyle: CalendarStyle(
              ...
              todayDecoration: BoxDecoration(
                color: Colors.purple, // 오늘 날짜도 배경 원 제거
                shape: BoxShape.circle,
              ),
              selectedDecoration: BoxDecoration(
                color: Colors.purple, // 선택된 날짜 강조 색상
                shape: BoxShape.circle,
              ),
            ...
            calendarBuilders: CalendarBuilders(
              defaultBuilder: (context, day, focusedDay) {
                ...
                if (_emotions.containsKey(day)) {
                  return Center(
                    child: _emotions[day], // 이모지 텍스트 맵에서 가져오기
                  );
                }
                return Center(
                  child: Text(
              ...
              selectedBuilder: (context, day, focusedDay) {
                return Container(
                  decoration: const BoxDecoration(
                    shape: BoxShape.circle,
                    color: Colors.orange, // 선택된 날짜 원 색상
                  ),
