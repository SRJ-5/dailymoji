...
class _SplashPageState extends State<SplashPage> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  ...
  // 애니메이션이 이미 시작되었는지 추적하는 플래그
  // 중복 애니메이션 시작을 방지하기 위함
  ...
  void _startAnimation() {
    if (!_hasStartedAnimation) {
      _hasStartedAnimation = true; // 시작됨을 표시
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed && mounted) {
      // 이용약관 페이지에서 뒤로가기로 돌아왔을 때 처리
      // 스플래시 애니메이션이 완료되지 않았다면 즉시 완료시킴
      if (!_controller.isCompleted) {
        _controller.forward(); // 현재 위치에서 끝까지 애니메이션 실행
      }
    }
  }
